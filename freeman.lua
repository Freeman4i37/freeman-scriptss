...
-- ===== SUPORTE PNG / DECAL / IMAGE =====

local function CreatePNGEntity(id)

    local model = Instance.new("Model")
    model.Name = config.Entity.Name or "PNG Entity"
    ...
    decal.Texture = "rbxassetid://".. tostring(id)
...
if typeof(asset) == "number" then
    ...
    success = true
    entityModel = CreatePNGEntity(asset)

elseif typeof(asset) == "string" and asset:match("^%d+$") then
...
elseif typeof(asset) == "string" and asset:find("rbxassetid://") then
    ...
    success = true
    entityModel = CreatePNGEntity(id)

elseif typeof(asset) == "Instance" and asset:IsA("Model") then
...
if success and entityModel then
    ...
    local entityTable = {
        ...
        SetCallback = function(self, key, callback)
            if self.Debug[key] and typeof(callback) == "function" then
