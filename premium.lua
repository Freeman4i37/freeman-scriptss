...
local tagMap = {
    ...
    ["Sub 👑"] = {users=subOwnerUsers, color=Color3.fromRGB(255,0,0)},
    ["Modder 🛡"] = {users=modderUsers, color=Color3.fromRGB(128,0,0)},
    ["Staff 🛠"] = {users=staffUsers, color=Color3.fromRGB(0,255,0)},
    ["Advanced 🎩"] = {users=advancedUsers, color=Color3.fromRGB(0,255,255)},
    ["Manager 🧑‍💼"] = {users=managerUsers, color=Color3.fromRGB(255,255,255)},
    ["Premium 💎"] = {users=premiumUsers, color=Color3.fromRGB(0,120,255)},
}
local tagType, tagColor = nil, nil
for k,v in pairs(tagMap) do
    if v.users[player.Name] then
        tagType = k
        tagColor = v.color
        break
    end
...
local musicIDs = {
    ...
    ["29"] = 112068892721408,
    ["30"] = 112143944982807, ["31"] = 111668097052966, ["32"] = 112214814544629, ["33"] = 101500915434329,
    ["34"] = 95046091312570, ["35"] = 110091098283354, ["36"] = 17422156627, ["37"] = 82411642961457,
}
local musicNames = {
    ...
    ["29"]="FUNK DA PRAIA (SLOWED)",["30"]="Hogo Funk",["31"]="Novinha sapeca",["32"]="ANALOG HORROR FUNK",["33"]="Dum Dum",["34"]="Rebola pro pai",["35"]="Carro Bixo",["36"]="Onichan",["37"]="Arrepia XL 6",
}
...
local function showTagForPlayer(plr)
    local tagType_, tagColor_ = nil, nil
    for k,v in pairs(tagMap) do
        if v.users[plr.Name] then
            tagType_ = k
            tagColor_ = v.color
            break
        end
    ...
    local function addTag()
        ...
        txt.Text = tagType_ or ""
        txt.TextColor3 = tagColor_ or gold
...
local function setMainInputsVisible(val)
    ...
    clInfoFrame.Visible = val and clInfoFrame.Visible -- só aparece se está em CL e tocando
end
